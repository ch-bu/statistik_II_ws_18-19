...
Stelle dir vor, du bist beim deutschen Konsortium der PISA-Studien eingestellt und hast einen Datensatz einer Schule erhalten. In dem Datensatz befindet sich eine Stichprobe von 20 Schülern der Schule. Du hast erfahren, dass diese Schule ein neues vielversprechendes Lehrkonzept umsetzt. Daher erwatest du, dass diese Schüler besser unterrichtet werden als im Mittel an deutschen Schulen. Nun möchtest du prüfen, ob die SuS in dieser Schule bessere Noten als im Durchschnitt in Deutschland erhalten. Die Noten sind in diesem Datensatz zwischen 0 und 15 Punkten skaliert (15 = sehr gut, 0 = ungenügend). Der Datensatz heißt `students.csv`.
...
students_cleaned <- students %>%
  select(school, sex, age, G3, studytime)
```

Wie viele Personen sind eigentlich im Datensatz? Zähle die Anzahl der Personen mit `nrow`:
...
ggplot(students_cleaned, aes(x = age)) +
  geom_bar()
```
...
Fällt unser Mittelwert in den roten Bereich, dann ..
